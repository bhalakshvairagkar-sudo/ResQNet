...
data class IncidentResponse(
    ...
    @SerializedName("ambulance") val assignedAmbulance: String? = null,
    @SerializedName("hospital") val assignedHospital: String? = null,
...
data class IncidentDto(
    ...
    @SerializedName("assignedAmbulance") val assignedAmbulance: String? = null,
...
data class EmergencyAlertDto(
    ...
    val priority: String? = null, val severity: Int? = null, val confidence: Float? = null,
    val accidentType: String? = null, val accidentTime: String? = null,
    val helpMessage: String? = null, val accidentLatitude: Double? = null, val accidentLongitude: Double? = null,
    val locationQuality: String? = null, val gpsAccuracy: Float? = null,
    val gForce: Float? = null, val speedDeltaKmh: Float? = null, val rollover: Boolean? = null,
    val patientMedicalInfo: String? = null, val mapUrl: String? = null,
    val distanceKm: Double? = null, val etaMinutes: Int? = null, val incomingAmbulance: String? = null,
    val assignedHospital: String? = null, val patientCount: Any? = null
)
...
interface ResQNetApi {
    ...
    @GET("api/incidents/alerts/pending") suspend fun getPendingAlerts(@Header("Authorization") authorization: String): Response<List<EmergencyAlertDto>>
    @POST("api/incidents/{id}/accept") suspend fun acceptDispatch(@Path("id") id: String, @Header("Authorization") authorization: String): Response<IncidentResponse>
    @POST("api/incidents/{id}/reject") suspend fun rejectDispatch(@Path("id") id: String, @Header("Authorization") authorization: String, @Body body: Map<String, String>): Response<IncidentResponse>
    @POST("api/incidents/{id}/hospital-ack") suspend fun acknowledgeHospitalAlert(@Path("id") id: String, @Header("Authorization") authorization: String): Response<IncidentResponse>
    @GET("api/ambulances/{id}") suspend fun getAmbulance(@Path("id") id: String, @Header("Authorization") authorization: String): Response<AmbulanceDto>
    @GET("api/hospitals/{id}") suspend fun getHospital(@Path("id") id: String, @Header("Authorization") authorization: String): Response<HospitalDto>
    @GET("api/incidents/mine") suspend fun getMyIncidents(@Header("Authorization") authorization: String): Response<List<IncidentDto>>
...
object ApiClient {
    // Android Studio emulators reach the development machine through 10.0.2.2.
    // A physical phone can select LOCAL_LAN in the dashboard after launch.
    // A physical phone needs the PC's LAN address; 10.0.2.2 works only in an emulator.
